<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">{{ __('Create Body Template') }}</h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-6xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <form method="POST" action="{{ route('bodies.store') }}" id="templateForm">
                    @csrf

                    <div class="mb-6">
                        <x-input-label for="name" :value="__('Template Name')" />
                        <x-text-input id="name" name="name" type="text" class="mt-1 block w-full" :value="old('name')" required placeholder="e.g., Welcome Email" />
                    ...
                    <div class="mb-6">
                        ...
                        <select name="body_group_id" id="body_group_id" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                            ...
                            @foreach($groups as $group)
                                <option value="{{ $group->id }}" {{ old('body_group_id') == $group->id ? 'selected' : '' }}>{{ $group->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <!-- Summernote Editor -->
                            <textarea id="html_content" name="html_content" class="summernote-editor">{{ old('html_content') }}</textarea>
                    ...
                    <div class="mt-6 mb-6">
                        ...
                        <textarea id="plain_content" name="plain_content" rows="4" class="mt-1 block w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm font-mono text-sm" placeholder="Plain text version for email clients that don't support HTML...">{{ old('plain_content') }}</textarea>
