            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                <form method="POST" action="{{ route('smtp.update', $smtp) }}">
                    ...
                    <div class="mb-4">
                        ...
                        <x-text-input id="name" name="name" type="text" class="mt-1 block w-full" :value="old('name', $smtp->name)" required />
                    ...
                    <div class="grid grid-cols-2 gap-4 mb-4">
                        ...
                        <div>
                            ...
                            <x-text-input id="from_email" name="from_email" type="email" class="mt-1 block w-full" :value="old('from_email', $smtp->from_email)" required />
                    ...
                    <div class="grid grid-cols-2 gap-4 mb-4">
                        <div>
                            ...
                            <x-text-input id="active_time_start" name="active_time_start" type="time" class="mt-1 block w-full" :value="old('active_time_start', $smtp->active_time_start ? substr($smtp->active_time_start, 0, 5) : '')" />
                            <x-input-error :messages="$errors->get('active_time_start')" class="mt-2" />
                        </div>
                        <div>
                            <x-input-label for="active_time_end" :value="__('Active End Time')" />
                            <x-text-input id="active_time_end" name="active_time_end" type="time" class="mt-1 block w-full" :value="old('active_time_end', $smtp->active_time_end ? substr($smtp->active_time_end, 0, 5) : '')" />
                    ...
                    <div class="mb-6">
                        <label class="flex items-center">
                            <input type="checkbox" name="is_active" value="1" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500" {{ old('is_active', $smtp->is_active) ? 'checked' : '' }}>
                    ...
                    <div class="flex items-center justify-between gap-4">
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <x-primary-button>{{ __('Update SMTP') }}</x-primary-button>
                        </div>
                    </div>
    ...
    <script>
        document.getElementById('testSmtpBtn').addEventListener('click', async function() {
            ...
            if (!password) {
                ...
                resultDiv.innerHTML = `<div class="flex items-start"><div class="flex-shrink-0"><svg class="h-5 w-5 text-yellow-500" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor"><path fill-rule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clip-rule="evenodd" /></svg></div><div class="ml-3"><p class="text-sm font-medium text-yellow-800">Please enter the password to test connection.</p></div></div>`;
            ...
            if (!data.host || !data.port || !data.username || !data.from_email || !data.from_name) {
                resultDiv.className = 'mt-4 border-l-4 border-yellow-500 bg-yellow-50 p-4 rounded-md shadow-sm text-yellow-800';
