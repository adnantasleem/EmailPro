...
class SmtpConfigController extends Controller
{
    ...
    public function store(Request $request)
    {
        $validated = $request->validate([
            ...
            'min_emails_per_day' => 'required_if:pacing_strategy,per_day|nullable|integer|min:1|max:100000',
            'max_emails_per_day' => 'required_if:pacing_strategy,per_day|nullable|integer|min:1|max:100000|gte:min_emails_per_day',
            'min_emails_per_hour' => 'required_if:pacing_strategy,per_hour|nullable|integer|min:1|max:100000',
            'max_emails_per_hour' => 'required_if:pacing_strategy,per_hour|nullable|integer|min:1|max:100000|gte:min_emails_per_hour',
    ...
    public function update(Request $request, SmtpConfig $smtp)
    {
        ...
        if ($validated['pacing_strategy'] === 'per_day') {
            if ($smtp->pacing_strategy !== 'per_day' || 
                ...
                $validated['current_daily_limit'] = rand($validated['min_emails_per_day'], $validated['max_emails_per_day']);
                $validated['limit_calculated_at'] = null; // Force recalculation of hourly limit
            }
            if (!empty($validated['max_emails_per_day'])) {
                $validated['daily_limit'] = $validated['max_emails_per_day'];
            }
        } elseif ($validated['pacing_strategy'] === 'per_hour') {
            if ($smtp->pacing_strategy !== 'per_hour' || 
                $smtp->min_emails_per_hour != $validated['min_emails_per_hour'] || 
                $smtp->max_emails_per_hour != $validated['max_emails_per_hour']) {
                $validated['limit_calculated_at'] = null; // Force recalculation of hourly limit
            }
            if (empty($validated['daily_limit'])) {
                $validated['daily_limit'] = null; // No hard limit for per_hour pacing
            }
        }
